...
//Rare exceptions to indicate a bug in the _wtcob code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_wtcob = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_wtcob]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.wtcob.module"
